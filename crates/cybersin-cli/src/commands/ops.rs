...
pub async fn execute(
    ...
    run_tui(daemon, model, gateway_ctx).await
}
...
async fn run_tui(
    ...
    let ui_result =
        tokio::task::spawn_blocking(move || run_terminal_loop(&ui_shared, &ui_status, action_tx))
            .await;
    refresh_task.abort();
    action_task.abort();
...
async fn run_gateway_actions(
    ...
    while let Some(action) = action_rx.recv().await {
        ...
        *status.lock().unwrap() = Some(message);
        if let Ok(model) = OpsModel::load(&daemon).await {
...
fn run_terminal_loop(
    ...
    let result = Terminal::new(backend)
        ...
        .and_then(|mut terminal| tui_loop(&mut terminal, shared, status, action_tx));
    let cleanup = (|| -> Result<()> {
        disable_raw_mode()?;
...
fn tui_loop(
    ...
    let titles = ["Sessions", "Traces", "Cost", "Approvals"];
    const APPROVALS_TAB: usize = 3;
    let mut selected = 0;
    let mut approval_cursor = 0usize;
    let mut pending: Option<PendingConfirm> = None;
    loop {
        let (pages, approval_lines, status_text) = {
            ...
            (pages, approval_lines, status.lock().unwrap().clone())
        };
        terminal.draw(|frame| {
