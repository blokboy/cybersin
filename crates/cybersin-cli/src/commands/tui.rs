...
fn ensure_agent_for_prompt_source(project_root: &Path, source: &Path) -> Result<PathBuf, String> {
    ...
    fs::write(&agent_path, scaffold_agent_yaml(&agent_name))
        .map_err(|e| format!("error: failed to write {}: {e}", agent_path.display()))?;
    Ok(agent_path)
}
...
fn scaffold_agent_yaml(agent_name: &str) -> String {
    format!(
...
mod tests {
    ...
    fn selected_build_compiles_that_prompt_and_ops_reads_dist_build_log() {
        ...
        assert!(project.path().join("agents/second.agent.yaml").exists());
